    ...
    for (int i = 0; i < 3; i++) {
        ...
        for (int w = 0; w < 4; w++) {
            if (leo_CheckCollisionCircleRec(state->balls[i].position, state->balls[i].radius, state->walls[w])) {
                if (w < 2) state->balls[i].velocity.y *= -1; // top/bottom
                else state->balls[i].velocity.x *= -1;       // left/right
            }
        }
        
        // Ball-ball collisions
        for (int j = i + 1; j < 3; j++) {
            if (leo_CheckCollisionCircles(state->balls[i].position, state->balls[i].radius, 
                                        state->balls[j].position, state->balls[j].radius)) {
    ...
    for (int i = 0; i < 2; i++) {
        ...
        for (int w = 0; w < 4; w++) {
            if (leo_CheckCollisionRecs(state->rects[i].rect, state->walls[w])) {
                if (w < 2) state->rects[i].velocity.y *= -1; // top/bottom
                else state->rects[i].velocity.x *= -1;       // left/right
            }
        }
...
static void demo_render_ui(leo_GameContext *ctx) {
    ...
    // Draw walls
    for (int i = 0; i < 4; i++) {
        leo_DrawRectangle((int)state->walls[i].x, (int)state->walls[i].y, 
                         (int)state->walls[i].width, (int)state->walls[i].height, LEO_GRAY);
    }
    
    // Draw balls
    for (int i = 0; i < 3; i++) {
        leo_DrawCircle((int)state->balls[i].position.x, (int)state->balls[i].position.y, 
                      state->balls[i].radius, state->balls[i].color);
    }
    
    // Draw rectangles
    for (int i = 0; i < 2; i++) {
        leo_DrawRectangle((int)state->rects[i].rect.x, (int)state->rects[i].rect.y, 
                         (int)state->rects[i].rect.width, (int)state->rects[i].rect.height, 
                         state->rects[i].color);
    }
    ...
    for (int i = 0; i < 3; i++) {
        if (leo_CheckCollisionPointCircle(mousePos, state->balls[i].position, state->balls[i].radius)) {
            ...
            leo_DrawCircle((int)state->balls[i].position.x, (int)state->balls[i].position.y, 
                          state->balls[i].radius + 5, LEO_WHITE);
            leo_DrawCircle((int)state->balls[i].position.x, (int)state->balls[i].position.y, 
                          state->balls[i].radius + 3, LEO_BLACK);
        }
    }
    ...
    for (int i = 0; i < 2; i++) {
        if (leo_CheckCollisionPointRec(mousePos, state->rects[i].rect)) {
            ...
            leo_Rectangle outline = {
                state->rects[i].rect.x - 3, 
                state->rects[i].rect.y - 3, 
                state->rects[i].rect.width + 6, 
                state->rects[i].rect.height + 6
            };
            leo_DrawRectangle((int)outline.x, (int)outline.y, 
                             (int)outline.width, (int)outline.height, LEO_WHITE);
        }
    }
    
    // Draw instructions (using default font)
