...
void leo_init_windows_dll_search(void)
{
    ...
    PFN_AddDllDirectory pAddDllDirectory = (PFN_AddDllDirectory)GetProcAddress(k32, "AddDllDirectory");

    if (pSetDefaultDllDirectories && pAddDllDirectory)
    {
        // Search app dir + user dirs (which we add below) + system dirs. No CurrentDir.
        pSetDefaultDllDirectories(LOAD_LIBRARY_SEARCH_APPLICATION_DIR | LOAD_LIBRARY_SEARCH_USER_DIRS |
