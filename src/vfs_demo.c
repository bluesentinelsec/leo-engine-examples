...
static void test_mount_pack(VFSState *state)
{
    ...
    if (success)
    {
        ...
        snprintf(state->status_text, sizeof(state->status_text), "✅ Successfully mounted resources.leopack");
        printf("✅ Successfully mounted resources.leopack\n");
    }
    else
    {
        snprintf(state->status_text, sizeof(state->status_text), "❌ Failed to mount resources.leopack");
...
static void test_asset_listing(VFSState *state)
{
    ...
    const char *test_assets[] = {
        "maps/map.json", "music/music.wav", "images/background_320x200.png", "images/character_64x64.png",
        "font/font.ttf", "sound/coin.wav"};
    ...
    snprintf(state->status_text, sizeof(state->status_text), "Asset Check: %d/%d found", found_count, total_count);
}
...
static void test_asset_loading(VFSState *state)
{
    ...
    if (content)
    {
        ...
        snprintf(state->status_text, sizeof(state->status_text), "✅ Successfully loaded and read map.json (%zu bytes)",
                 size);
    }
    else
    {
        printf("❌ Failed to load maps/map.json\n");
        snprintf(state->status_text, sizeof(state->status_text), "❌ Failed to load map.json");
    }
}
...
static void test_streaming_api(VFSState *state)
{
    ...
    if (stream)
    {
        ...
        snprintf(state->status_text, sizeof(state->status_text), "✅ Streaming API works - opened %zu byte audio file",
                 info.size);
    }
    else
    {
        printf("❌ Failed to open music/music.wav for streaming\n");
        snprintf(state->status_text, sizeof(state->status_text), "❌ Streaming API failed");
    }
}
...
static void demo_update(leo_GameContext *ctx)
{
    ...
    if (leo_IsKeyPressed(KEY_ESCAPE) || (state->one_frame && ctx->frame >= 300) ||
