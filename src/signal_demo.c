...
static void demo_update(leo_GameContext *ctx) {
    ...
    // Update flash timers
    if (state->button_node.flash_timer > 0) state->button_node.flash_timer -= ctx->dt;
    if (state->timer_node.flash_timer > 0) state->timer_node.flash_timer -= ctx->dt;
    if (state->counter_node.flash_timer > 0) state->counter_node.flash_timer -= ctx->dt;
    if (state->logger_node.flash_timer > 0) state->logger_node.flash_timer -= ctx->dt;
...
static void demo_render_ui(leo_GameContext *ctx) {
    ...
    snprintf(info, sizeof(info), "Timer: %.1fs %s", 
             state->timer_accumulator, state->timer_paused ? "(PAUSED)" : "");
    leo_DrawText(info, 10, 70, 16, LEO_WHITE);
    
    snprintf(info, sizeof(info), "Connections: %s", 
             state->connections_active ? "ACTIVE" : "DISABLED");
    ...
    int start_idx = (state->log_count > MAX_LOG_ENTRIES) ? 
                    state->log_count - MAX_LOG_ENTRIES : 0;
