...
static void init_particles(ZeldaDemoState *state)
{
    for (int i = 0; i < 1000; i++)
    {
        ...
        if (color_choice == 0)
            state->particles[i].color = (leo_Color){139, 69, 19, 180}; // Brown
        else if (color_choice == 1)
            state->particles[i].color = (leo_Color){255, 215, 0, 180}; // Gold
        else
            state->particles[i].color = (leo_Color){255, 140, 0, 180}; // Orange
    }
}
...
static void update_particles(ZeldaDemoState *state, float dt)
{
    for (int i = 0; i < 1000; i++)
    {
        ...
        // Wrap horizontally
        if (p->x < -10)
            p->x = state->map->width * 32.0f + 10;
        if (p->x > state->map->width * 32.0f + 10)
            p->x = -10;
    }
}
...
static void render_particles(ZeldaDemoState *state)
{
    ...
    for (int i = 0; i < 1000; i++)
    {
        ...
        if (p->x >= cam_bounds.x - 10 && p->x <= cam_bounds.x + cam_bounds.width + 10 && p->y >= cam_bounds.y - 10 &&
            p->y <= cam_bounds.y + cam_bounds.height + 10)
        {
...
static void check_single_enemy_collision(leo_Actor *actor, void *user_data)
{
    ...
    if (strcmp(leo_actor_name(actor), "enemy") != 0)
        return;
...
static void player_render(leo_Actor *self)
{
    ...
    if (data->alive)
    {
        ...
        {
            leo_DrawTextureRec(state->hero_texture, (leo_Rectangle){0, 0, 32, 32}, (leo_Vector2){data->x, data->y},
                               LEO_WHITE);
        }
    }
}
...
static void enemy_update(leo_Actor *self, float dt)
{
    ...
    if (new_x < data->origin_x - max_distance)
        new_x = data->origin_x - max_distance;
    if (new_x > data->origin_x + max_distance)
        new_x = data->origin_x + max_distance;
    if (new_y < data->origin_y - max_distance)
        new_y = data->origin_y - max_distance;
    if (new_y > data->origin_y + max_distance)
        new_y = data->origin_y + max_distance;
...
static void enemy_render(leo_Actor *self)
{
    ...
    if (is_in_camera_bounds(cam_bounds, data->x, data->y, 32))
    {
        leo_DrawTextureRec(state->enemy_texture, (leo_Rectangle){0, 0, 32, 32}, (leo_Vector2){data->x, data->y},
                           LEO_WHITE);
    }
}
...
static void demo_update(leo_GameContext *ctx)
{
    ...
    {
        leo_SetWindowMode(LEO_WINDOW_MODE_WINDOWED);
    }
    if (leo_IsKeyPressed(KEY_2))
    {
        leo_SetWindowMode(LEO_WINDOW_MODE_BORDERLESS_FULLSCREEN);
    }
    if (leo_IsKeyPressed(KEY_3))
    {
        leo_SetWindowMode(LEO_WINDOW_MODE_FULLSCREEN_EXCLUSIVE);
    }
...
static void demo_render_ui(leo_GameContext *ctx)
{
    ...
    if (state->map)
    {
        ...
        if (start_y < 0)
            start_y = 0;
        if (end_x >= state->map->width)
            end_x = state->map->width - 1;
        if (end_y >= state->map->height)
            end_y = state->map->height - 1;
