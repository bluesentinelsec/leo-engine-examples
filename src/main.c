...
int main(int argc, char *argv[])
{
    ...
    static struct option long_options[] = {
        {"help", no_argument, NULL, 'h'},        {"list-demos", no_argument, NULL, 'l'},
        {"index", required_argument, NULL, 'i'}, {"cicd", no_argument, NULL, 'c'},
        {"all", no_argument, NULL, 'a'},         {NULL, 0, NULL, 0}};
    ...
    int c;
    while ((c = getopt_long(argc, argv, "hli:ca", long_options, &option_index)) != -1)
    {
        switch (c)
        {
        case 'h':
    ...
    int action_count = (list_demos ? 1 : 0) + (selected_index >= 0 ? 1 : 0) + (run_all ? 1 : 0);
    ...
    if (selected_index >= 0)
    {
        ...
        printf("Running demo %d: %s\n", selected_index, gDemos[selected_index].name);
