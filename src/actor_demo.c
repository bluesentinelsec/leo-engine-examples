...
static const leo_ActorVTable player_vtable = {
    .on_init = player_init, .on_update = player_update, .on_render = player_render, .on_exit = NULL};

static const leo_ActorVTable enemy_vtable = {
    .on_init = enemy_init, .on_update = enemy_update, .on_render = enemy_render, .on_exit = enemy_exit};

static const leo_ActorVTable particle_vtable = {
    .on_init = particle_init, .on_update = particle_update, .on_render = particle_render, .on_exit = particle_exit};
...
static void player_update(leo_Actor *self, float dt)
{
    ...
    // Movement
    if (leo_IsKeyDown(KEY_W))
        data->y -= 200 * dt;
    if (leo_IsKeyDown(KEY_S))
        data->y += 200 * dt;
    if (leo_IsKeyDown(KEY_A))
        data->x -= 200 * dt;
    if (leo_IsKeyDown(KEY_D))
        data->x += 200 * dt;
...
static bool demo_setup(leo_GameContext *ctx)
{
    ...
    leo_ActorDesc player_desc = {
        .name = "player", .vtable = &player_vtable, .user_data = NULL, .groups = 0, .start_paused = false};
    state->player_actor = leo_actor_spawn(ctx->root, &player_desc);

    // Spawn initial enemies
    for (int i = 0; i < 3; i++)
    {
        leo_ActorDesc enemy_desc = {.name = "enemy",
                                    .vtable = &enemy_vtable,
                                    .user_data = NULL,
...
static void demo_update(leo_GameContext *ctx)
{
    ...
    if (leo_IsKeyPressed(KEY_SPACE))
    {
        for (int i = 0; i < 5; i++)
        {
            leo_ActorDesc particle_desc = {.name = "particle",
...
static void demo_render_ui(leo_GameContext *ctx)
{
    ...
    snprintf(info, sizeof(info), "Global Pause: %s", leo_actor_system_is_paused(ctx->actors) ? "ON" : "OFF");
