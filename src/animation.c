...
static bool demo_setup(leo_GameContext *ctx)
{
    ...
    state->animation = leo_LoadAnimation("images/animation_32x32_4_frames.png", // VFS path (no "resources/" prefix)
                                         32,                                    // frame width
                                         32,                                    // frame height
                                         4,                                     // frame count
    ...
    printf("✅ Loaded animation from VFS: %dx%d, %d frames\n", state->animation.frameWidth,
           state->animation.frameHeight, state->animation.frameCount);
...
static void demo_update(leo_GameContext *ctx)
{
    ...
    if (leo_IsKeyPressed(KEY_UP))
    {
        ...
        if (state->speed_multiplier > 3.0f)
            state->speed_multiplier = 3.0f;
    }
    if (leo_IsKeyPressed(KEY_DOWN))
    {
        state->speed_multiplier -= 0.5f;
        if (state->speed_multiplier < 0.1f)
            state->speed_multiplier = 0.1f;
    }
...
static void demo_render_ui(leo_GameContext *ctx)
{
    ...
    // Animation info
    char info[128];
    snprintf(info, sizeof(info), "Frame: %d/%d", state->player.currentFrame + 1, state->animation.frameCount);
    ...
    snprintf(info, sizeof(info), "Status: %s", state->player.playing ? "Playing" : "Paused");
