...
static bool demo_setup(leo_GameContext *ctx)
{
    ...
    {
        state->obstacles[i].rect =
            (leo_Rectangle){(float)(rand() % (WORLD_WIDTH - 100) + 50), (float)(rand() % (WORLD_HEIGHT - 100) + 50),
                            (float)(40 + rand() % 60), (float)(40 + rand() % 60)};
    }
    ...
    for (int i = 0; i < MAX_COLLECTIBLES; i++)
    {
        state->collectibles[i] = (Collectible){
            {(float)(rand() % (WORLD_WIDTH - 100) + 50), (float)(rand() % (WORLD_HEIGHT - 100) + 50)}, 8.0f, false};
    }
...
static void demo_render_ui(leo_GameContext *ctx)
{
    ...
    for (int i = 0; i < MAX_OBSTACLES; i++)
    {
        ...
        leo_DrawRectangle((int)r.x, (int)r.y, (int)r.width, (int)r.height, (leo_Color){101, 67, 33, 255}); // Brown
    }
    ...
    leo_DrawRectangle((int)state->player.x, (int)state->player.y, (int)state->player.width, (int)state->player.height,
                      LEO_RED);
